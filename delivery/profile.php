<?php
/**
 * delivery/profile.php — Delivery partner profile + password management.
 *
 * 1. Profile form (name, email, phone, address, avatar upload)
 * 2. Change password form
 * 3. Stats summary: total deliveries, delivered count
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($formType === 'profile') {
        ...
        redirect(APP_URL . '/delivery/profile.php');
    }
...
// ─── Stats ────────────────────────────────────────────────────────────
$totalAssigned = (int) db_scalar('SELECT COUNT(*) FROM deliveries WHERE partner_id = ?', [$uid]);
$totalDelivered = (int) db_scalar("SELECT COUNT(*) FROM deliveries WHERE partner_id = ? AND status = 'delivered'", [$uid]);
$totalActive = (int) db_scalar(
    "SELECT COUNT(*) FROM deliveries WHERE partner_id = ? AND status IN ('assigned','picked_up','in_transit')",
    [$uid]
);
...
<div class="row g-3">
    ...
    <div class="col-lg-6">
        <div class="card mb-3">
            ...
            <div class="card-body">
                <form method="post" action="<?= APP_URL ?>/delivery/profile.php" enctype="multipart/form-data">
                    <?= csrf_field() ?>
    ...
    <div class="col-lg-6">
        <div class="card mb-3">
            ...
            <div class="card-body">
                <form method="post" action="<?= APP_URL ?>/delivery/profile.php">
                    <?= csrf_field() ?>
        ...
        <div class="card">
            ...
            <div class="card-body small text-muted">
                <ul class="mb-0 ps-3">
                    <li>Always confirm the recipient name before handing over an order.</li>
                    <li>Take a clear proof photo showing the parcel and address.</li>
                    <li>Log out from shared devices after your shift ends.</li>
                </ul>
            </div>
        </div>
    </div>
